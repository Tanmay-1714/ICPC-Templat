...
void dfs1(int node, int edgeID, int parent, vector<vector<pair<int,int>>>& adj, int& n, vector<bool>& visited, vector<int>& component_number, int& current_number, vector<bool>& isBridge) {
    visited[node] = true;
    component_number[node] = current_number;
    for(auto [child, id] : adj[node]) {
        if(id == edgeID) continue;
        if(!visited[child] && !isBridge[id]) {
            dfs1(child, id, node, adj, n, visited, component_number, current_number, isBridge);
        }
    }
}
...
int32_t main()
{
    ...
    while (tt--) {
        ...
        // now perform second dfs 
        // this time treat bridge edges as if they dont exist and map every node to a component number
        ...
        for(int i = 1; i <= n; i++) {
            if(visited[i]) continue;
            dfs1(i, 0, -1, adj, n, visited, component_number, current_number, isBridge);
            current_number++;
        }

        vector<vector<int>> new_adj(current_number);

        for(auto bridge : answer) {
            int u = bridge[0];
            int v = bridge[1];
            assert(component_number[u] != component_number[v]);
            new_adj[component_number[u]].push_back(component_number[v]);
            new_adj[component_number[v]].push_back(component_number[u]);
        }

        // so the bridge tree consists of nodes numbered from 1 to final value of current number - 1
        // and new_adj is the adj list of the bridge tree
        debug(current_number);
        debug(component_number);
